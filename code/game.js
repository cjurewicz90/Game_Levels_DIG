...
function Level(plan) {
  ...
  Ghost.prototype.speed = new Vector(this.player.xSpeed * 0.25, this.player.xSpeed * 0.25);
}
...
Ghost.prototype.act = function(step, level) {
	var direction = new Vector(level.player.pos.x - this.pos.x, level.player.pos.y - this.pos.y);
	this.speed = new Vector(magnitude(this.speed) * direction.x / magnitude(direction), magnitude(this.speed) * direction.y / magnitude(direction));
	this.pos = new Vector(this.pos.x + this.speed.x * step, this.pos.y + this.speed.y * step);
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if (type.indexOf("lava") != -1 && this.status == null && !playerInvulnerable) {
    this.status = "lost";
    this.finishDelay = 1;
  } 
  else if (type == "ghost" && this.status == null && !playerInvulnerable) {
	  this.status = "lost";
	  this.finishDelay = 1;
  }
  else if (type == "coin") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
		...
		this.player.type = "powerUp";
		this.player.xSpeed *= 2;
		this.player.jumpSpeed *= 1.5;
...
function runLevel(level, Display, andThen) {
  ...
  runAnimation(function(step) {
	...
	//else if (level.player.type.indexOf("powerUp") != -1) {
	else if(level.player.type == "powerUp" || frameCountPowerUp > 0) {
		playerInvulnerable = true;
		
		if (frameCountPowerUp % 2 == 0) {
			// var ch = level.player.type[level.player.type.length - 1];
			// var num = ((parseInt(ch) + 1) % 5 + 1).toString();
			// level.player.type = "powerUp" + num;
			if(level.player.type == "powerUp") {
				level.player.type = "player";
			} else {
				level.player.type = "powerUp";
			}
		}
		
		if (frameCountPowerUp == 250) {
			frameCountPowerUp = 0;
			level.player.type = "player";
			
			level.player.xSpeed /= 2;
			level.player.jumpSpeed /= 1.5;
